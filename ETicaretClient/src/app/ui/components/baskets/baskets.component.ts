...
export class BasketsComponent extends BaseComponent implements OnInit {
  ...
  shoppingComplete() {
    ...
    this.dialogService.openDialog({
      ...
      afterClosedWithResult: async (result) => {
        if (!result) return;
        this.showSpinner(SpinnerType.BallAtom);
        ...
        if (result.district || result.city) {
            fullAddress = `${result.city} / ${result.district} - ${result.address}`;
        }
        
        order.address = fullAddress || "Belirtilmemiş Adres";
        order.description = "Müşteri özel bir adres tanımladı.";
